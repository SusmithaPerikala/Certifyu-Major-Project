...
app.post(
  ...
  [
    ...
    check("regd", "Only 10 characters")
      ...
      .matches(
        /^[20-5]{2}[b0|B0]{2}[1|5]{1}[a|A]{1}[0|1]{1}[1-5]{1}[0-9a-zA-Z]{1}[0-9]{1}$/
      )
    ...
    check("pwd")
      ...
      .isLength({ min: 8 })
      .withMessage("Give atleast 8 characters")
      .matches(/(?=.*?[A-Z])/)
  ...
  (req, res) => {
    ...
    if (!errors1.isEmpty()) {
      //isEmpty()  === True ---> no errors --> else is executed
      ...
      details.get().then((doc) => {
        if (doc.exists) {
          // Document already exists, you may want to handle this case accordingly
          res.render("stu_SL", {
            
            showAlert: 1,
          });
        } else {
          // Document does not exist, proceed with setting its data
...
app.post('/facultySignup', [
  ...
  check('facid', 'Only 3 characters')
          .isLength({ min: 3, max: 3 }).matches(/^[0-9]{3}$/).withMessage('Invalid ID'),
